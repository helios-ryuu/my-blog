...
void main() {
  ...
  if (uEnableMouse > 0.5) {
    ...
    float d2 = dot(md, md);
    // Subtle, compact cursor light glow on threads
    bright += clamp(uMouseStrength, 0.0, 1.0) * uMouseActive * exp(-d2 * 28.0) * 0.5;
    // Delicate soft core illumination around cursor
    vec3 cursorGlow = mix(uColor1, uColor2, 0.5);
    col += cursorGlow * (exp(-d2 * 32.0) * 0.35 + exp(-d2 * 14.0) * 0.12) * uMouseActive * clamp(uMouseStrength, 0.0, 1.0);
    gsum += exp(-d2 * 24.0) * 0.15 * uMouseActive;
  }
  col *= bright;
